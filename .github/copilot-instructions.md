...
- **AI capabilities: plugins vs. skills (two distinct sources)**: `setup/config/copilot/settings.json` declaratively installs Copilot-specific *plugins* from `scaryrawr/scarypilot` through `extraKnownMarketplaces` and `enabledPlugins`. `setup/ai/setup-skills.sh` installs portable *skills* from `scaryrawr/agentic` via `gh skill install ... --scope user --agent <agent>`. Azure DevOps is provided to Copilot by `azure-devops@scarypilot`; `blogify` and `image-gen` are provided by `omlx-media@scarypilot`. Keep installing those three agentic skills for non-Copilot agents, but do not install duplicate copies for Copilot. Leave unrelated personal skills in `setup-skills.sh`.
...
- **Herdr integrations run after agent setup**: Keep `setup/ai/setup-zz-herdr-integrations.sh` ordered after the Pi and Copilot setup scripts. Herdr requires their config directories—and specifically `~/.pi/agent/extensions`—to exist before its integration installers run.
