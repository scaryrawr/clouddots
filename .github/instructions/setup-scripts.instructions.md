...
4. **Plugin installation**: For git-based shell/editor plugins, clone if the directory is missing and `git pull` if it already exists. Install Copilot plugins and their native extensions declaratively through the ScaryPilot marketplace in `setup/config/copilot/settings.json`; do not clone extension repositories into `$HOME/.copilot/extensions/`.
...
14. **Bash covers two shell surfaces**: When adding an env var or PATH entry for bash in `setup/shells/setup-bash.sh`, mirror it into **both** `~/.bashrc` (interactive) **and** `~/.bashenv` (non-interactive, loaded via `BASH_ENV`). A setting only in `.bashrc` is invisible to non-interactive shells. zsh's `~/.zshenv` and fish's universal `fish_user_paths` already cover both surfaces, so no mirroring is needed there. For PATH entries in `.bashenv`, guard against duplicate prepends, e.g. `[[ -d "$HOME/.local/bin" && ":$PATH:" != *":$HOME/.local/bin:"* ]] && export PATH=...`.
